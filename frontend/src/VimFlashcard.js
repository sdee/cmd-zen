...
function VimFlashcard(props) {
    ...
    return (
      <>
        {input.split("").map((ch, i) => {
          ...
          if (status === "wrong") {
            color = "#e53e3e";
          } else if (ch === current.shortcut[i]) {
            color = "#38a169";
          } else {
            color = "#e53e3e";
          }
          return <span key={i} style={{ color, fontWeight: "bold" }}>{ch}</span>;
        })}
      </>
    );
  }
  ...
  return (
    ...
    >
      {showStats ? (
        <Statistics onBack={() => setShowStats(false)} />
      ) : (
        <>
          ...
          <div style={{ marginTop: 24 }}>
            <a
              href="#"
              style={{ color: "#32cd32", textDecoration: "none", fontFamily: "monospace" }}
              onClick={(e) => {
                e.preventDefault();
                setShowStats(true);
              }}
